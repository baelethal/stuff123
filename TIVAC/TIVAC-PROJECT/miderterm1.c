...
Tiva C Midterm
November 4, 2017
This program connects the TIVA C board to a lux sensor and the nodeMCU.
It reads data from the sensor and sends it to the nodeMCU and then the
data is sent to a thingspeak server.
*/
#include <stdarg.h>
#include <stdbool.h>
#include <stdint.h>
...
//Configures the UART to run at 19200 baud rate
{
    ...
    GPIOPinConfigure(GPIO_PB0_U1RX);                // configures PB0 as RX
    GPIOPinConfigure(GPIO_PB1_U1TX);                // configures PB1 as TX
    GPIOPinTypeUART(GPIO_PORTB_BASE, GPIO_PIN_0 | GPIO_PIN_1);  // sets UART pin type

    // sets uart clock, boudrate, and which uart to use
    UARTConfigSetExpClk(UART1_BASE, SysCtlClockGet(), 115200,
        (UART_CONFIG_WLEN_8 | UART_CONFIG_STOP_ONE | UART_CONFIG_PAR_NONE));
}

// prints uart command string
void UARTCommand(unsigned char *str) {
    char i=0;
    while(str[i] != '\0') {
...
void UARTInt(int n) {
    ...
    while(count>=0) {
        UARTCharPut(UART1_BASE, uartChar[count]);
        count--;
    }
}

// calculates the number of chars to print int
int IntLen(int n) {
    int count=0;
    while(n > 0) {
        n/=10;
...
//Initializes the TSL2591 to have a medium gain,
{
    ...
    //used during debuging to make sure correct ID is received
     UARTprintf ("Found Device! %i", x);
    }
    else
    {
     UARTprintf ( "Device not Found! %i", x);
...
//Initializes the MCU
{
    UARTCommand("AT+RST\r\n");      // resets the MCU
    SysCtlDelay(200000);
    UARTCommand("AT+CWMODE=1\r\n"); // sets MCU to station mode
    SysCtlDelay(20000);
    UARTCommand("AT+CWJAP=\"Zcolo123-2Ghz\",\"DataCenter123#\"\r\n");   // connects MCU to wifi router
    SysCtlDelay(200000);
    UARTCommand("AT+CIPMUX=0\r\n");                                     // sets MCU to single connection
    SysCtlDelay(20000);
}

void timer0_init ()
//Initializes the timer0 to 30 seconds
{
    uint32_t ui32Period;

    // enable timer0 and port F
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOF);
    GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3);
    SysCtlPeripheralEnable(SYSCTL_PERIPH_TIMER0);
    TimerConfigure(TIMER0_BASE, TIMER_CFG_PERIODIC);

    // calculate timer period for 30 seconds
    ui32Period = (SysCtlClockGet() * 30);
...
void main (void)
{
    ...
    MCU_init();         //initialize the nodeMCU
...
void Timer0IntHandler(void)
{
    ...
    // connect to thingspeak using TCP connection
    UARTCommand("AT+CIPSTART=\"TCP\",\"184.106.153.149\",80\r\n");
    SysCtlDelay(200000);

    // calculate and send how many characters are necessary
    UARTCommand("AT+CIPSEND=");
    SysCtlDelay(20);
    UARTInt(72+IntLen(luxAvg));
    SysCtlDelay(20);
    UARTCommand("\r\n");
    SysCtlDelay(20000);

    // send data to thingspeak
    UARTCommand("GET https://api.thingspeak.com/update?api_key=0XLLUY55C4GFKPH7&field1=");
    SysCtlDelay(20);
    UARTInt(luxAvg);
    SysCtlDelay(20);
    UARTCommand("\r\n");
